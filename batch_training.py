...
dnet = DilatedNet(256, 256, classes, use_ctx_module=True)
print(dnet.output_shape)
try:
    print('Loading previous weights')
    dnet.load_weights('dnet.h5')
except:
    print('Training from scratch !!!')
p_dnet = multi_gpu_model(dnet, 4)
p_dnet.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['mse','accuracy'])
tb = TensorBoard(log_dir='logs', write_graph=True)
mc = ModelCheckpoint(mode='max', filepath='dnet_model/top_weightsd.h5', monitor='acc', save_best_only='True', save_weights_only='True', verbose=2)
es = EarlyStopping(mode='max', monitor='acc', patience=6, verbose=1)
callbacks = [tb, mc, es]
train_gen = seg_gen(image_list, mask_list, batch_size)


p_dnet.fit_generator(train_gen, steps_per_epoch=steps, epochs=10, callbacks=callbacks, workers=8)
print('Saving final weights')
dnet.save_weights('dnet_model/dnet.h5')
